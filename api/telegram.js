...
Current mission: Get FORGE its first real user.`);
    ...
    if (/^scan:/i.test(userText) || /^find:/i.test(userText)) {
      ...
      await redisSet(`axis:pending:${chatId}`, { action: 'scan', query }, 300);
      return new Response('OK', { status: 200 });
    }
    ...
    if (/^browse:/i.test(userText)) {
      ...
      await redisSet(`axis:pending:${chatId}`, { action: 'browse', platform, query }, 300);
      return new Response('OK', { status: 200 });
    }

    // ━━━ LEVEL 4 — APPROVE PENDING ACTION ━━━
    if (/^confirm execute$/i.test(userText)) {
      const pending = await redisGet(`axis:pending:${chatId}`);
      if (!pending) {
      ...
      await redisSet(`axis:pending:${chatId}`, null, 1);
...
export default async function({ page }) {
  ...
  const results = await page.evaluate(() => {
    const items = [];
    const selectors = ['article', '.post', '.story', 'li', '.thread', '[class*="post"]'];
    ...
    for (const selector of selectors) {
      ...
      elements.forEach((el, i) => {
        ...
        if (text && text.length > 40 && text.length < 1000) {
          items.push({ 
