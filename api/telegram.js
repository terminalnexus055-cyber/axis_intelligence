...
async function executeBrowserTask(pending, chatId) {
  ...
  try {
    // Multi-platform scan — run in parallel, collect all results
    const platforms = [
      { name: 'indiehackers', url: 'https://www.indiehackers.com/group/freelancers' },
      { name: 'hackernews', url: `https://hn.algolia.com/api/v1/search?query=${encodeURIComponent(query + ' freelance')}&tags=story&numericFilters=created_at_i>${Math.floor(Date.now()/1000) - 7776000}` }
    ];

    console.log('Multi-platform scan starting...');

    // Run all platform scans in parallel
    const scanResults = await Promise.allSettled(
      platforms.map(p => scanPlatform(p.name, p.url, query))
    );
    ...
    scanResults.forEach((result, i) => {
      if (result.status === 'fulfilled' && result.value.length > 0) {
        ...
        console.log(`${platforms[i].name}: ${result.value.length} items`);
      } else {
        platformsFailed.push(platforms[i].name);
        console.log(`${platforms[i].name}: failed or empty`);
      }
    });

    if (allItems.length === 0) {
      return { error: `No results found across all platforms. Platforms tried: ${platforms.map(p => p.name).join(', ')}` };
    }

    // Format for Groq analysis
    const postsContext = allItems.map((item, i) =>
...
STRICT FILTERING:
- ONLY include people ASKING FOR HELP or EXPRESSING PAIN
- REJECT anyone promoting themselves or selling services
- REJECT posts older than 6 months
- Pain indicators: "struggling", "can't find", "no clients", "help", "frustrated", "failing", "0 revenue", "burned out", "lost", "stuck"

Return TOP 10 genuine pain cases. For each:
- Number, pain score (1-10), one sentence pain summary, one sentence human outreach opener, URL

Keep each entry concise. Never fabricate. End with: "Reply 'outreach [number]' for full message."`,
      'Commander',
      []
    );

    return {
      report: `🌐 *MULTI-PLATFORM INTEL COMPLETE*\n\n✅ Scanned: ${platformsScanned.join(', ')}\n${platformsFailed.length > 0 ? `⚠️ Failed: ${platformsFailed.join(', ')}\n` : ''}📊 Total posts analyzed: ${allItems.length}\n\n━━━━━━━━━━━━━━━━\n\n${analysis}`
    };
...
async function scanPlatform(platformName, url, query) {
  // HN uses API — direct JSON fetch
  if (platformName === 'hackernews') {
    try {
      ...
      return hits.slice(0, 15).map(h => ({
        ...
        date: h.created_at ? new Date(h.created_at).toLocaleDateString() : 'Unknown'
      })).filter(h => h.text.length > 10);
    } catch { return []; }
  }

  // All other platforms — use Browserless
  ...
  const content = await page.evaluate(() => {
    ...
    document.querySelectorAll('a').forEach(el => {
      ...
      const u = el.href || '';
      const isPost = u.includes('/post/') || u.includes('/product/') || u.includes('/group/');
  ...
  try {
    ...
    return items.map(item => ({
      platform: platformName === 'indiehackers' ? 'IndieHackers' : platformName,
      text: item.text || '',
      url: item.url || url,
      date: 'Recent'
