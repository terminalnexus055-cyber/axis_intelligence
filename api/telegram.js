...
Current mission: Get FORGE its first real user.`);
    ...
    if (/^execute (browse|scan) (\S+) (\S+)/i.test(userText)) {
      ...
      const platformUrls = {
        ...
        'reddit': `https://old.reddit.com/r/freelance/search/?q=${encodeURIComponent(query)}&sort=new&restrict_sr=1`,
        'freelancers': `https://old.reddit.com/r/freelancers/search/?q=${encodeURIComponent(query)}&sort=new&restrict_sr=1`,
        'hackernews': `https://news.ycombinator.com/search?q=${encodeURIComponent(query)}`
      };
...
async function executeBrowserTask(pending, chatId) {
  ...
  try {
    ...
    // Always use Puppeteer — direct fetch blocked by Reddit/most platforms
    let text = '';
