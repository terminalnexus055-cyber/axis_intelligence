...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (hasPhoto || hasVideo) {
      ...
      // Fetch video and store in Redis for renderer access
      if (hasVideo) {
        ...
        if (videoFile?.file_id) {
          ...
          // Also fetch and store video binary for renderer
          try {
            ...
            if (videoUrl) {
              ...
              await redisSet(`axis:media:${videoMediaId}`, videoBase64, 3600);
              await redisSet('axis:video:mediaId', videoMediaId, 3600);
              console.log('Video stored in Redis:', videoMediaId);
            }
          } catch (err) {
...
Current mission: Get FORGE its first real user.`);
    ...
    if (/^video$/i.test(userText) || /^make video$/i.test(userText)) {
      ...
      // Get video URL — use Vercel-served URL so renderer can access it
...
async function generateVideoScript(pending) {
  ...
  // Analyze video frames if we have a video URL
  let frameAnalysis = null;
  const videoFileId = await redisGet('axis:video:fileId');
  if (videoFileId && mediaType === 'video') {
    const videoUrl = await getTelegramFileUrl(videoFileId);
    if (videoUrl) {
