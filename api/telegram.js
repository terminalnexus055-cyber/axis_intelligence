...
Current mission: Get FORGE its first real user.`);
    ...
    if (/^execute (browse|scan) (\S+) (\S+)/i.test(userText)) {
      ...
      const platformUrls = {
        'indiehackers': 'https://www.indiehackers.com/group/main-forum',
...
async function executeBrowserTask(pending, chatId) {
  ...
  const content = await page.evaluate(() => {
    ...
    document.querySelectorAll('a').forEach(el => {
      ...
      const u = el.href || '';
      const isPost = u.includes('/post/') || u.includes('/product/') || u.includes('/group/') || u.includes('ycombinator');
    ...
    if (items.length === 0) {
      items.push({ text: document.body.innerText.slice(0, 3000), url: window.location.href });
    }
