...
Current mission: Get FORGE its first real user.`);
    ...
    if (/^video$/i.test(userText) || /^make video$/i.test(userText)) {
      ...
      await sendMessage(chatId, '🎬 *Video production started*\n\nGenerating script → voiceover → submitting to render queue...\n\nType `video status` in 60 seconds to get your download link.');
      await sendTyping(chatId);

      // Run async — don't await
      produceVideo(pending, chatId).catch(err => {
        console.error('Async video error:', err.message);
      });
    ...
    // ━━━ VIDEO STATUS CHECK ━━━
    if (/^video status$/i.test(userText)) {
      const projectId = await redisGet('axis:video:project');
      if (!projectId) {
        await sendMessage(chatId, '⚠️ No video render in progress. Type `video` first.');
        return res.status(200).json({ ok: true });
      }
      ...
      const url = pollData?.movie?.url;
      if (status === 'done' && url) {
        await redisSet('axis:video:project', null, 1);
        await sendMessage(chatId, `🎬 *Video Ready*\n\n${url}\n\n_Download and post directly._`);
      } else if (status === 'error') {
        await sendMessage(chatId, `⚠️ Render failed. Try \`video\` again.`);
      } else {
        await sendMessage(chatId, `⏳ Still rendering (status: ${status || 'queued'}). Try again in 30 seconds.`);
      }
      return res.status(200).json({ ok: true });
    }
...
async function produceVideo(pending, chatId) {
  try {
    ...
    if (!audioUrl) {
      await sendMessage(chatId, '⚠️ Voiceover generation failed. Check EIDOS_API_KEY.');
      return;
    }

    const projectId = await submitVideoRender(audioUrl, script, pending.caption);
    if (!projectId) {
      await sendMessage(chatId, '⚠️ Video render submission failed. Check JSON2VIDEO_API_KEY.');
      return;
    }

    // Save project ID for status polling
    await redisSet('axis:video:project', projectId, 3600);
    await sendMessage(chatId, `✅ *Render submitted*\n\nProject: \`${projectId}\`\n\nType \`video status\` to check when ready.`);

  } catch (err) {
    console.error('produceVideo error:', err.message);
    await sendMessage(chatId, `⚠️ Video production error: ${err.message}`);
  }
}
...
async function submitVideoRender(audioUrl, script, caption) {
  try {
    ...
    const data = await response.json();
    const projectId = Array.isArray(data) ? data[0]?.project : data?.project;
    if (!projectId) {
      console.error('JSON2Video no project ID:', JSON.stringify(data));
      return null;
    }

    console.log('JSON2Video render submitted:', projectId);
    return projectId;
