...
async function produceVideo(pending) {
  try {
    ...
    // Step 2 — Generate voiceover via Fish Audio
    ...
    // Step 3 — Assemble video via Creatomate
    const videoUrl = await assembleVideo(audioUrl, script, pending.caption);
    if (!videoUrl) return { error: 'Video assembly failed. Check CREATOMATE_API_KEY.' };
...
async function generateVoiceover(script) {
  try {
    ...
    if (!response.ok) {
      console.error('Fish Audio failed:', response.status);
      return null;
    }

    // Fish Audio returns binary audio — upload to a temp host
    const audioBuffer = await response.arrayBuffer();
    const base64Audio = Buffer.from(audioBuffer).toString('base64');

    // Store audio in Redis temporarily (24hr) and return as data URI for Creatomate
    const audioKey = `axis:audio:${Date.now()}`;
    await redisSet(audioKey, base64Audio, 86400);

    // Return base64 data URI — Creatomate accepts this
...
async function assembleVideo(audioUrl, script, caption) {
  try {
    ...
    if (!response.ok) {
      console.error('Creatomate failed:', response.status, await response.text());
      return null;
    }
    ...
    for (let i = 0; i < 10; i++) {
      ...
      console.log('Creatomate status:', pollData.status);
      if (pollData.status === 'succeeded') return pollData.url;
