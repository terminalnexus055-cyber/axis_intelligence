...
Current mission: Get FORGE its first real user.`);
    ...
    if (/^video$/i.test(userText) || /^make video$/i.test(userText)) {
      ...
      await sendMessage(chatId, '🎬 *Generating script and submitting render...*\n\nThis takes 10-15 seconds.');
      await sendTyping(chatId);
      ...
      try {
        ...
        await sendMessage(chatId, `✅ *Render submitted*\n\nProject: \`${projectId}\`\n\nType \`video status\` in 30-60 seconds to get your download link.\n\nScript: _"${script}"_`);

      } catch (err) {
        console.error('Video error:', err.message);
        await sendMessage(chatId, `⚠️ Video error: ${err.message}`);
      }
