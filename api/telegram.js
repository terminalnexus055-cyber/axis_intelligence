...
async function executeBrowserTask(pending, chatId) {
  ...
  try {
    ...
    const platforms = [
      ...
      { 
        name: 'HackerNews',
        url: `https://hn.algolia.com/api/v1/search_by_date?query=${encodeURIComponent('freelance clients struggling help')}&tags=ask_hn,story&numericFilters=created_at_i>${fiveDaysAgo}`
      }
    ];
    ...
    const analysis = await callGroq(
      `Analyze these posts. Find FREELANCERS with genuine pain. Query: "${query}"

POSTS:
${postsContext}

FORGE targets: Freelancers who sell their skills/services (designers, developers, writers, marketers, consultants) — NOT SaaS founders or product builders.

HARD REJECT these immediately:
- Anyone building a SaaS, app, or software product
- Anyone with AWS bills, hosting costs, or technical infrastructure problems  
- Success stories or milestone posts
- People promoting their tools or services
- Job postings or hiring posts
- Posts older than 5 days

ONLY ACCEPT:
- Freelancers struggling to find clients
- Freelancers dealing with inconsistent income
- Freelancers being ghosted by clients
- Freelancers unsure how to price their work
- Freelancers feeling burned out or lost

For each genuine freelancer in pain:
[Number]. Pain: X/10 | [Their exact struggle in one sentence] | [Human opener referencing their specific situation — no pitch] | [URL]

If zero genuine freelancers exist in these posts, say exactly: "No genuine freelancer pain found this scan. Try again later."
Never fabricate.`,
      'Commander',
      []
    );
    ...
    try {
      ...
      return hits
        .filter(h => {
          ...
          if (postDate <= fiveDaysAgoMs) return false;
          // Must be freelance related
          const title = (h.title || '').toLowerCase();
          const freelanceWords = ['freelan', 'client', 'contract', 'upwork', 'fiverr', 'remote work', 'self employ'];
          return freelanceWords.some(w => title.includes(w));
        })
        .slice(0, 15)
        .map(h => ({
          platform: 'HackerNews',
