...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (hasPhoto || hasVideo) {
      ...
      // Store video file ID for later render use
      if (hasVideo) {
...
Current mission: Get FORGE its first real user.`);
    ...
    if (/^video$/i.test(userText) || /^make video$/i.test(userText)) {
      ...
      // Get video URL from pending if media was uploaded
      const videoFileId = await redisGet('axis:video:fileId');
      if (videoFileId) {
        script.videoUrl = await getTelegramFileUrl(videoFileId);
      }
...
The scene structure must emerge from the emotional arc — not from a fixed template.
Model the hook on what's converting this week.

Return ONLY a valid JSON object, no markdown, no explanation:
{
  "voiceover": "full 30-second spoken script here — natural, conversational, max 80 words",
  ...
  "scenes": [
    ...
    },
    {
      ...
      "duration": 8,
      "subtitle": "what voiceover says during this scene",
      "zoom": true
    },
    {
      "type": "stat_overlay",
      "duration": 4,
      "text": "punchy stat or insight"
    },
    {
      "type": "screen_recording",
      "duration": 11,
...
SCENE TYPE RULES:
- text_only: bold text on dark gradient, no video needed
- screen_recording: uses Mohammed's uploaded recording as visual
- stat_overlay: blurred frame from recording + bold text overlay
- cta: dark gradient + branding

IMPORTANT:
- Scene count and order must serve the emotional arc
- Durations must add up to exactly 30 seconds
- voiceover must be speakable in 30 seconds (max 80 words)
